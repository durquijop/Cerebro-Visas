...
const EMBEDDING_DIMENSIONS = 256 // Dimensiones del vector
...
function keywordsToVector(keywords, originalText) {
  ...
  // Generar vector usando hash de palabras
  for (const word of allWords) {
    const hash = simpleHash(word)
    const index = Math.abs(hash) % EMBEDDING_DIMENSIONS
    const value = ((hash % 1000) / 1000) // Normalizar entre -1 y 1
    vector[index] += value
    
    // Agregar variación a índices cercanos para capturar semántica
    const index2 = (index + 1) % EMBEDDING_DIMENSIONS
    const index3 = (index + EMBEDDING_DIMENSIONS - 1) % EMBEDDING_DIMENSIONS
    vector[index2] += value * 0.5
    vector[index3] += value * 0.5
  }
