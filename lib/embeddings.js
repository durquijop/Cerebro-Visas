...
Formato: palabra1, palabra2, palabra3, ...`
    ...
    if (!response.ok) {
      ...
      console.error('Error en OpenAI (model: gpt-4.1-mini):', error)
 ...
 */
export function chunkTextWithPages(text, pageTexts = null, maxChunkSize = 4000, overlap = 200) {
  const chunks = []
  ...
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      try {
        ...
        // Rate limiting más agresivo para evitar 429
        if (i < chunks.length - 1) {
          await new Promise(r => setTimeout(r, 3000)) // 3 segundos entre embeddings
        }
      } catch (embError) {
