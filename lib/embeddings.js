 ...
 * VERSIÓN 3: OpenRouter para embeddings (OpenAI para chat)
 */
 ...
 * Nota: Usamos OpenRouter para embeddings porque el proyecto OpenAI 
 * no tiene acceso al modelo de embeddings
 */
export async function generateEmbedding(text) {
  const OPENROUTER_API_KEY = process.env.OPENROUTER_API_KEY
  
  if (!OPENROUTER_API_KEY) {
    // Fallback: intentar con cálculo simple si no hay API key
    console.warn('OPENROUTER_API_KEY no configurada, embeddings no disponibles')
