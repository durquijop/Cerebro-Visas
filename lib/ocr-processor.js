...
Devuelve solo el texto extraído:`
        ...
        if (!response.ok) {
          const statusCode = response.status
          if ((statusCode === 503 || statusCode === 502 || statusCode === 429 || statusCode === 500) && retryCount < MAX_RETRIES - 1) {
            retryCount++
        ...
        if (error.name === 'AbortError') {
          ...
          success = true // No reintentar timeouts
        } else {
          retryCount++
    ...
    // Pequeña pausa entre lotes exitosos
