...
const MAX_PAGES_PER_REQUEST = 2  // Solo 2 páginas por request para evitar problemas
const PAGE_TIMEOUT = 120000      // 2 minutos
const MAX_RETRIES = 3
...
async function convertPdfToImages(pdfBuffer, totalPages) {
  ...
  try {
    ...
    try {
      ...
      // Usar 72 DPI para imágenes más pequeñas y manejables
      await execAsync(`pdftoppm -jpeg -r 72 -jpegopt quality=70 "${pdfPath}" "${outputPrefix}"`, { timeout: 120000 })
      
      // Leer las imágenes generadas (ahora son JPEG)
      const files = await fs.readdir(tempDir)
      const imgFiles = files.filter(f => f.endsWith('.jpg') || f.endsWith('.png')).sort()
      
      for (let i = 0; i < imgFiles.length; i++) {
        const imgPath = path.join(tempDir, imgFiles[i])
...
async function processImagesWithVision(images, totalPages, apiKey) {
  ...
  for (let i = 0; i < images.length; i += MAX_PAGES_PER_REQUEST) {
    ...
    while (!success && retryCount < MAX_RETRIES) {
      try {
        // Detectar el tipo MIME basado en el contenido base64
        // JPEG empieza con /9j/, PNG empieza con iVBOR
        const isJpeg = batch[0].base64.startsWith('/9j/')
        ...
        const content = [
          {
            type: 'text',
            text: `You are an OCR assistant. Your task is to extract ALL visible text from these ${batch.length} document image(s).

Instructions:
1. Extract EVERY word, number, and symbol exactly as shown
2. Include all dates, reference numbers, names, and addresses
3. Mark each page with: "=== PAGE ${batch[0].pageNumber} ===" (and so on)
4. Do NOT summarize or interpret - copy the EXACT text
5. If text is unclear, write [illegible]
