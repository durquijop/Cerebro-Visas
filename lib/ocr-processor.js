  ...
  for (let i = 0; i < images.length; i += MAX_PAGES_PER_REQUEST) {
    ...
    while (!success && retryCount < MAX_RETRIES) {
      try {
        ...
        // Construir contenido con imágenes - prompt muy simple y neutral
        const content = [
          {
            type: 'text',
            text: `Please transcribe all text visible in ${batch.length > 1 ? 'these images' : 'this image'}. Copy every word, number and symbol exactly as shown. Output only the transcribed text.`
          }
        ]
