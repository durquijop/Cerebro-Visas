...
export async function saveStructuredData(supabase, documentId, structuredData) {
  try {
    ...
    if (document_info) {
      ...
      if (updateError) {
        console.error('   ⚠️ Error actualizando documento:', updateError.message)
      }
    }
    ...
    if (issues && issues.length > 0) {
      ...
      const { error: bulkError } = await supabase.from('document_issues').insert(issueRecords)
      
      if (bulkError) {
        console.error('   ⚠️ Error bulk insert issues:', bulkError.message)
        console.log('   🔄 Intentando insertar issues uno por uno...')
        
        // Si falla el bulk (ej: FK constraint), intentar uno por uno
        for (const record of issueRecords) {
          const { error: singleError } = await supabase.from('document_issues').insert(record)
          if (singleError) {
            console.log(`      ⚠️ Issue ${record.taxonomy_code} falló: ${singleError.message}`)
          } else {
            issuesSaved++
          }
        }
        console.log(`   ✓ Issues guardados individualmente: ${issuesSaved}/${issueRecords.length}`)
      } else {
        issuesSaved = issueRecords.length
        console.log(`   ✓ Issues guardados: ${issuesSaved}`)
      }
    }
    ...
    if (requests && requests.length > 0) {
      ...
      const { error: reqError } = await supabase.from('document_requests').insert(requestRecords)
      
      if (reqError) {
        console.error('   ⚠️ Error bulk insert requests:', reqError.message)
        console.log('   🔄 Intentando insertar requests uno por uno...')
        
        for (const record of requestRecords) {
          const { error: singleError } = await supabase.from('document_requests').insert(record)
          if (singleError) {
            console.log(`      ⚠️ Request falló: ${singleError.message}`)
          } else {
            requestsSaved++
          }
        }
        console.log(`   ✓ Requests guardados individualmente: ${requestsSaved}/${requestRecords.length}`)
      } else {
        requestsSaved = requestRecords.length
        console.log(`   ✓ Requests guardados: ${requestsSaved}`)
      }
    }

    return { success: true, issuesSaved, requestsSaved }
  } catch (error) {
