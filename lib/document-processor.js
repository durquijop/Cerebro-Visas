/**
 * Document Processor v5 - SOLUCIÓN DEFINITIVA
 * 
 * ESTRATEGIA:
 * 1. PDFs con texto → pdf-parse (instantáneo)
 * 2. PDFs escaneados → pdftoppm + GPT-4o Vision OCR
 * 
 * Conversión PDF→Imágenes: usa pdftoppm (poppler-utils)
 * OCR: GPT-4o Vision (confiable, no requiere facturación extra)
 */
...
async function convertPdfToImages(pdfBuffer, pdftoppmPath) {
  ...
  try {
    ...
    // Convertir a JPEG (150 DPI, calidad 80 - buen balance)
    const cmd = `${pdftoppmPath} -jpeg -r 150 -jpegopt quality=80 "${pdfPath}" "${outputPrefix}"`
...
/**
 * Realiza OCR usando GPT-4o Vision
 */
...
- If text is unclear, make your best interpretation`
    ...
    try {
      ...
      if (lowerText.includes("can't assist") || 
        ...
        console.log(`      ⚠️ Páginas ${pageNums}: GPT-4o rechazó (probando prompts alternativos)`)
